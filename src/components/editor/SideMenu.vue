<template>
  <div class="container">
    <div class="menu">
      <div v-for="item in menu" :key="item.value" :class="['menu-item', item.value === current ? 'active' : '']">
        <div class="left"></div>
...
const MENU = [
  ...
  { label: '转场', value: 'transition', icon: transitionIcon },
  { label: '覆盖', value: 'overlay', icon: overlayIcon }
];
export default {
  data() {
    return {
      menu: MENU,
      current: MENU[1].value
    };
  }
}
</script>
