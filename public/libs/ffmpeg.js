function ffmpeg(customModule) {
    var Module = Object.assign({}, customModule);
    var moduleOverrides = {};
    var key;
    for (key in Module) {
    ...
    var wasmBinaryFile = location.href.indexOf('dist') > -1 ? "/New-Video/dist/libs/ffmpeg.wasm" : "../libs/ffmpeg.wasm";
    ...
    var FS = {
        ...
        closeStream: function(fd) {
            const currentStream = FS.streams.filter((stream)=>{
                return stream && stream.fd === fd;
            });
            Module["onFilesChanged"] && Module["onFilesChanged"](currentStream[0]);
            FS.streams[fd] = null
        },
        chrdev_stream_ops: {
            open: function(stream) {
    ...
    var _ff_h264_cabac_tables = Module["_ff_h264_cabac_tables"] = 1539253;
